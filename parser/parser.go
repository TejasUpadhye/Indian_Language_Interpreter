...
func (p *Parser) parsePrintStatement() *ast.PrintStatement {
	...
	// Ensure '(' follows 'print'
	if !p.expectPeek(token.LPAREN) {
		p.errors = append(p.errors, "expected '(' after 'print'")
		return nil
	}

	// Parse the first argument (message)
	p.nextToken()
	stmt.Message = p.parseExpression(LOWEST)

	// Ensure ',' separates the arguments
	if !p.expectPeek(token.COMMA) {
		p.errors = append(p.errors, "expected ',' after the first argument")
		return nil
	}

	// Parse the second argument (variable)
	p.nextToken()
	stmt.Variable = p.parseExpression(LOWEST)

	// Ensure ')' closes the function call
	if !p.expectPeek(token.RPAREN) {
